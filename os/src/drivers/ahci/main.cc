...
class Ahci_device
{
	private:
		...
		{
			Ram_dataspace_capability ds = alloc_dma_mem(0x1000);
	...
	public:
		...
		Ram_dataspace_capability alloc_dma_mem(size_t size)
		{
			return _pci.alloc_dma_mem(_pci_device_cap, size);
		}
};
...
class Ahci_driver : public Block::Driver
{
	...
	public:
		...
		Ram_dataspace_capability alloc_dma_mem(size_t size)
		{
			return _device->alloc_dma_mem(size);
		}
};
