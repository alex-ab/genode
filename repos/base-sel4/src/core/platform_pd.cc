...
struct Pd_id_alloc : Platform_pd::Pd_id_allocator
{
	Pd_id_alloc()
	{
		 ...
		 */
		_reserve(Core_cspace::TOP_CNODE_CORE_IDX   , 1); /* 0x000 */
		_reserve(Core_cspace::CORE_VM_ID           , 1); /* 0x001 */
		_reserve(Core_cspace::TOP_CNODE_UNTYPED_16K, 1); /* 0xffd */
		_reserve(Core_cspace::TOP_CNODE_UNTYPED_4K , 1); /* 0xffe */
		_reserve(Core_cspace::TOP_CNODE_PHYS_IDX   , 1); /* 0xfff */
	}
};
