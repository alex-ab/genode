...
Trace::Execution_time Platform_thread::execution_time() const
{
	...
	unsigned quantum = Nova::Qpd::DEFAULT_QUANTUM;

	if (_cpu_quota) {
		uint64_t time_periodic = 0;
		uint64_t dummy = 0;
		res = Nova::sc_ec_time(_sel_sc_period(), _sel_sc_period(), time_periodic, dummy);
		if (res != Nova::NOVA_OK)
			warning("sc_ctrl periodic failed res=", res);
		sc_time += time_periodic;

		quantum += unsigned(Cpu_session::quota_lim_downscale<__uint128_t>(_cpu_quota, PERIOD_QUANTUM));
	}

	return { ec_time, sc_time, quantum, _priority };
}
