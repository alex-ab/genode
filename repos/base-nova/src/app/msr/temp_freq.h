...
struct Msr::Monitoring
{
	...
	Genode::uint64_t mperf { };
	Genode::uint64_t aperf { };
	Genode::uint64_t freq_khz { };
	bool             freq_khz_valid { };

	Genode::uint8_t temp_tcc           { };
	bool            temp_tcc_valid     { };
	Genode::uint8_t temp_package       { };
	bool            temp_package_valid { };
	Genode::uint8_t temp_cpu           { };
	bool            temp_cpu_valid     { };
	...
	void update_package_temperature(Nova::Utcb &utcb)
	{
		...
		temp_package       = Status::Temperature::get(status);
	...
	void update_cpu_temperature(Nova::Utcb &utcb)
	{
		...
		temp_cpu       = Status::Temperature::get(status);
	...
	void target_temperature(Nova::Utcb &utcb)
	{
		...
		temp_tcc       = Target::Temperature::get(status);
		temp_tcc_valid = success & (1 << pos);
	}

	void cpu_frequency(Nova::Utcb &utcb, Genode::uint64_t tsc_freq_khz)
	{
