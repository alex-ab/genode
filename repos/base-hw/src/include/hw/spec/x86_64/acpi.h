...
struct Hw::Acpi_fadt : Genode::Mmio
{
	...
	struct Pm1a_evt_blk : Register < 56, 32> { };
	struct Pm1b_evt_blk : Register < 60, 32> { };
	...
	struct Pm1_evt_len  : Register < 88,  8> { };
	struct Pm1_cnt_len  : Register < 89,  8> { };

	struct Pm1a_evt_blk_ext : Register < 148, 32> {
		struct Addressspace : Bitfield < 0, 8> { };
		struct Width        : Bitfield < 8, 8> { };
	};
	struct Pm1a_evt_blk_ext_addr : Register < 148 + 4, 64> { };

	struct Pm1b_evt_blk_ext : Register < 160, 32> {
		struct Addressspace : Bitfield < 0, 8> { };
		struct Width        : Bitfield < 8, 8> { };
	};
	struct Pm1b_evt_blk_ext_addr : Register < 160 + 4, 64> { };
	...
	unsigned read_event_enable()
	{
		auto constexpr half_register = true;
		auto constexpr half_offset   = true;

		auto const evt_a = _read<Pm1_evt_len, Pm1a_evt_blk,
		                         Pm1a_evt_blk_ext::Width,
		                         Pm1a_evt_blk_ext::Addressspace,
		                         Pm1a_evt_blk_ext_addr>(half_register,
		                                                half_offset);
		auto const evt_b = _read<Pm1_evt_len, Pm1b_evt_blk,
		                         Pm1b_evt_blk_ext::Width,
		                         Pm1b_evt_blk_ext::Addressspace,
		                         Pm1b_evt_blk_ext_addr>(half_register,
		                                                half_offset);

		return evt_a | evt_b;
	}

	unsigned read_event_status()
	{
		auto constexpr half_register = true;
	...
	void write_event_status(unsigned value)
	{
		auto constexpr half_register = true;

		_write<Pm1_evt_len, Pm1a_evt_blk, Pm1a_evt_blk_ext::Width,
		       Pm1a_evt_blk_ext::Addressspace, Pm1a_evt_blk_ext_addr>(value,
		                                                              half_register);
		_write<Pm1_evt_len, Pm1b_evt_blk, Pm1b_evt_blk_ext::Width,
		       Pm1b_evt_blk_ext::Addressspace, Pm1b_evt_blk_ext_addr>(value,
		                                                              half_register);
	}
	...
	unsigned _access(bool hw_read, unsigned value, bool half_register = false,
	...
	template <typename REG_LEN, typename BLK_ADDR, typename BLK_WIDTH_EXT,
	          typename BLK_AS_EXT, typename BLK_ADDR_EXT>
	void _write(unsigned value, bool half_register = false, bool half_offset = false)
	{
		enum { WRITE = false };
