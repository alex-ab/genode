		...
		struct Waiter : Thread
		{
			struct Sel /* NOVA kernel-capability selector */
			{
				...
				static Sel init_signal_sem(Thread &thread)
				{
					...
					return { exc_base + Novae::SM_SEL_SIGNAL };
				}
				...
				auto up()
				{
					return Novae::sm_ctrl(value, Novae::SEMAPHORE_UP);
				}
			};
			...
			void entry() override
			{
				...
				for (;;) {
					 ...
					 */
					if (_wakeup_sem.down(deadline_tsc()) == Novae::NOVA_TIMEOUT)
			...
			void update_deadline(Tsc const deadline)
			{
				...
				if (sooner_than_scheduled)
					if (_wakeup_sem.up() != Novae::NOVA_OK)
