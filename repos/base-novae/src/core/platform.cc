                                  ...
                                  auto &mem_ram)
{
	...
	auto map_size = align_addr((hip.uefi_mmap_addr() & 0xfffull)
	                           + hip.uefi_mmap_size(), 12);

	region_alloc.alloc_aligned(map_size, 12).with_result([&] (auto & local) {
		map = local.ptr;
                                  ...
                                  addr_t const phys_mbi)
{
	...
	region_alloc.alloc_aligned(0x2000, 13).with_result([&] (auto & local) {
		map = local.ptr;
		local.deallocate = false;
	}, [&] (auto) { /* !map is checked below */ });
	...
	[&](auto mod_start, auto mod_end, auto /* cmd */, auto /* cmd_size */) {
		...
		(void)mem_ram.remove_range(mod_start & ~0xffful, align_addr(mod_size, 12));
	...
	_vm_base(0x1000), _vm_size(0), _cpus(Affinity::Space(1,1))
{
	...
	(void)region_alloc().remove_range(CORE_PAGER_UTCB_ADDR - get_page_size(),
	                                  get_page_size() * 3);

	/* exclude UTCB of main thread + empty guard pages before and after */
	(void)region_alloc().remove_range((addr_t)__main_thread_utcb - get_page_size(),
	                                  get_page_size() * 3);

	/* exclude HIP */
	(void)region_alloc().remove_range(addr_t(&hip), align_addr(addr_t(hip.length()), 12));
