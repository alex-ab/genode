...
/*
 * Copyright (C) 2009-2026 Genode Labs GmbH
 *
...
#else
extern addr_t __initial_x0;
extern addr_t __initial_x1;
#endif
...
static void exception_handler(addr_t const pt_id)
{
	...
	auto const ip = utcb->ip();
	auto const sp = utcb->sp();

	if (pt_id == PT_SEL_PAGE_FAULT) {
		auto const pf_type = utcb->pf_type();
		auto const pf_addr = utcb->pf_addr();

		error("PAGE-FAULT IN CORE addr=", Hex(pf_addr), " ip=", Hex(ip));
		...
		error("EXCEPTION IN CORE ip=", Hex(ip), " sp=", Hex(sp),
		      " exception=", Hex(pt_id));
...
static void startup_handler(Genode::addr_t const utcb_new_global_ec)
{
	...
	reply(core_pager_stack_top(), Mtd::gpr_ip() | Mtd::sp());
}
                                         ...
                                         auto const boot_cpu)
{
	...
	for (unsigned pt_id = 0; pt_id < (1u << NUM_PT_ARCH_LOG2); pt_id++) {
		ret = create_pt(pt_id, core_pd_sel, ec_sel,
		                (addr_t)exception_handler);

		if (ret != Novae::NOVA_OK)
			error(__func__, ":", __LINE__, " returned ", ret);

		imprint_badge(core_cap_sel, pt_id, pt_id, Mtd::qual() | Mtd::sp() |
		                                          Mtd::gpr_ip());
	}
...
static bool cpuid_invariant_tsc()
{
	addr_t edx = 0;
#ifdef __x86_64__
	unsigned long cpuid = 0x80000007;
...
static bool parse_uefi_memory_map(auto &region_alloc, Hip const &hip,
                                  auto &mem_ram)
{
	if (hip.uefi_mmap_addr() == ~0ULL)
		return false;

	void * map = nullptr;

	auto map_size = align_addr((hip.uefi_mmap_addr() & 0xfffull)
	                           + hip.uefi_mmap_size(), AT_PAGE);

	region_alloc.alloc_aligned(map_size, AT_PAGE).with_result([&] (auto & local) {
		map = local.ptr;
	...
	Mem_crd src(hip.uefi_mmap_addr() >> 12, 0, Rights::read_only());
	Mem_crd dst(         addr_t(map) >> 12, 0, Rights::read_only());

	async_map(Core::Platform::kernel_host_sel(),
	          Core::Platform::core_host_sel(), src, dst);

	for (unsigned i = 4096; i < map_size; i += 4096) {
		Mem_crd src_tmp((hip.uefi_mmap_addr() + i) >> 12, 0, Rights::read_only());
		Mem_crd dst_tmp(         (addr_t(map) + i) >> 12, 0, Rights::read_only());

		async_map(Core::Platform::kernel_host_sel(),
		          Core::Platform::core_host_sel(), src_tmp, dst_tmp);
	}

	Genode::error(" mmap_addr=", Hex(hip.uefi_mmap_addr()), "+",
	                             Hex(hip.uefi_mmap_size()),
	              " desc=", Hex(hip.uefi_desc_size()),
	              " ver=", Hex(hip.uefi_desc_ver()), " -> ", map);
	...
	for (addr_t m = 0; m < hip.uefi_mmap_size(); m += hip.uefi_desc_size()) {
		auto offset = hip.uefi_mmap_addr() & 0xfffu;
		auto addr   = addr_t(map) + m + offset;

		auto &range = *reinterpret_cast<efi_memory_descriptor *>(addr);

		if (range.type != EfiConventionalMemory)
			error("remove me ", Hex(range.type), " ",
			      Hex_range(range.physicalstart, range.NumberOfPages << 12));

		if (range.type == EfiConventionalMemory)
			(void)mem_ram.add_range(range.physicalstart,
...
static bool parse_mbi2_memory_map(auto &mem_io, auto &mem_ram, auto &region_alloc,
                                  auto &core_phys_start, Boot_info &boot_info,
                                  addr_t const phys_mbi)
{
	void * map = nullptr;

	auto const offset = phys_mbi & 0xfffu;

	region_alloc.alloc_aligned(0x2000, Align { .log2 = 13 }).with_result([&] (auto & local) {
		map = local.ptr;
		local.deallocate = false;
	}, [&] (auto) { /* !map is checked below */ });
	...
	auto phys_next = offset ? align_addr(phys_mbi, AT_PAGE)
	                        : phys_mbi + 0x1000;
	...
	(void)mem_ram.remove_range(phys_mbi & ~0xffful,
	                           align_addr(phys_end - phys_mbi + offset,
	                                      AT_PAGE));
...
#ifndef __x86_64__
	{
		auto const aligned_base = 0x40000000ul; /* Qemu */
		auto const aligned_size = 0x10000000ul; /* Qemu 256M */

		(void)mem_io .remove_range(aligned_base, aligned_size);
		(void)mem_ram.add_range   (aligned_base, aligned_size);

		error("ram HARDCODED ", Hex(aligned_base), "+", Hex(aligned_size));
	}
#endif
	...
	mbi2.for_each_tag([&] (Multiboot2_info::Memory const & m) {
		...
		auto const aligned_base = align_addr(base, AT_PAGE);

		if (!size || (offset && (0x1000 - offset <= size)))
			return;

		auto const aligned_size = align_addr(size - (offset ? (0x1000 - offset) : 0), AT_PAGE);
	...
	mbi2.for_each_tag([&] (Multiboot2_info::Memory const & m) {
		...
		auto const aligned_size = align_addr(size + offset, AT_PAGE);
		...
		if (type == Multiboot2_info::Memory::Type::ACPI_RECLAIM_MEMORY ||
			...
			(void)mem_io.add_range(aligned_base, aligned_size);

		(void)mem_ram.remove_range(aligned_base, aligned_size);
	...
	[&] (Multiboot2_info::Framebuffer const &fb) {
		...
		(void)mem_ram.remove_range(phys_addr & ~0xffful,
		                           align_addr(phys_size + offset, AT_PAGE));
	...
	[&] (auto cmdline, auto size) {
		...
		(void)mem_ram.remove_range(phys_cmd & ~0xffful,
		                           align_addr(size + phys_offset, AT_PAGE));
	},
	[&](auto mod_start, auto mod_end, auto /* cmd */, auto /* cmd_size */) {
		...
		(void)mem_ram.remove_range(mod_start & ~0xffful, align_addr(mod_size, AT_PAGE));
...
#else
	auto const phys_mbi = __initial_x1;

	if (__initial_x0 != Multiboot2_info::MAGIC)
		return true; /* Multiboot v1 or other kinds of boot are unsupported */
#endif

	if (!parse_uefi_memory_map(region_alloc, hip, mem_ram))
		log("no UEFI memory map");

	if (!parse_mbi2_memory_map(mem_io, mem_ram, region_alloc, core_phys_start,
	                           boot_info, phys_mbi))
		log("no MBI2 memory map");
	...
	_vm_base(0x1000), _vm_size(0), _cpus(Affinity::Space(1,1))
{
	...
	enum { UART_VIRT_ADDR = 0x1000 };

	setup_uart(UART_VIRT_ADDR);
	...
	(void)region_alloc().remove_range(core_virt_beg, core_size);
	...
	/* preserve page used for UART in core's virtual address space */
	(void)region_alloc().remove_range(UART_VIRT_ADDR, 0x1000);

	/* preserve stack area in core's virtual address space */
	(void)region_alloc().remove_range(stack_area_virtual_base(),
	                                  stack_area_virtual_size());

	/* exclude UTCB of core pager thread + empty guard pages before and after */
	(void)region_alloc().remove_range(CORE_PAGER_UTCB_ADDR - PAGE_SIZE,
	                                  PAGE_SIZE * 3);

	/* exclude UTCB of main thread + empty guard pages before and after */
	(void)region_alloc().remove_range((addr_t)__main_thread_utcb - PAGE_SIZE,
	                                  PAGE_SIZE * 3);

	/* exclude HIP */
	(void)region_alloc().remove_range(addr_t(&hip), align_addr(addr_t(hip.length()), AT_PAGE));
	...
	error_memory = setup_allocators(_io_mem_alloc, ram_alloc(), region_alloc(),
	                                _core_phys_start, boot_info, hip);

	/* remove reserved RAM regions occupied by kernel */
	(void)ram_alloc().remove_range(hip.nova_addr_start(), hip.nova_addr_end() - hip.nova_addr_start());
	(void)ram_alloc().remove_range(hip.mbuf_addr_start(), hip.mbuf_addr_end() - hip.mbuf_addr_start());
	(void)ram_alloc().remove_range(hip.root_addr_start(), hip.root_addr_end() - hip.root_addr_start());

	/* needed as I/O memory by the VESA driver and acpi to search for rsdp */
	(void)_io_mem_alloc.add_range   (0, 0x2000);
	(void)ram_alloc()  .remove_range(0, 0x2000);
...
bool Mapped_mem_allocator::_map_local(addr_t virt_addr, addr_t phys_addr, size_t size)
{
	auto res = map_local(Platform::kernel_host_sel(),
	                     Platform::core_host_sel(),
	                     phys_addr, virt_addr, size / PAGE_SIZE,
	                     Rights::rw());
