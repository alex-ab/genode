...
void Irq_object::sigh(Signal_context_capability cap)
{
	...
	if (_irq_type == Irq_session::TYPE_LEGACY)
		ok = associate_gsi(irq_sel(), _idx, dc_sel(), _gsi_flags);
	else
		ok = associate_msi(irq_sel(), _idx, dc_sel(), _msi_addr, _msi_data);
...
Thread::Start_result Irq_object::start(unsigned irq, addr_t const bdf, Irq_args const &irq_args)
{
	...
	if (_irq_type != Irq_session::TYPE_LEGACY) {
		auto res = Novae::create_dc(dc_sel(), platform_specific().core_pd_sel(),
		                            bdf, 0 /* dtd1 */, 0 /* dtd2 */);
		if (res != Novae::NOVA_OK)
			return Start_result::DENIED;
	}
...
Irq_object::Irq_object(Runtime &runtime, Allocator::Result const &)
:
	Thread(runtime, "core_irq", Stack_size { 4096 }, { }),
	_kernel_caps(cap_map().insert(1))
{ }
...
Irq_object::~Irq_object()
{
	...
	with_native_thread([&] (Native_thread const &nt) {
		/* let thread die in-kernel by revoking pagefault selector */
		revoke(core_pd, Novae::Obj_crd(nt.exc_pt_sel +
	...
	while (_state == READY || _state == KILL) {
		...
		if (res != Novae::NOVA_OK)
			error(__func__, ":", __LINE__, " res=", res);

		/* in case irq thread is blocked in _wait_for_ack.block() */
		_wait_for_ack.wakeup();

		wait_for_irq_construction().block();
	}

	auto res = Novae::deassign_int(irq_sel());
	if (res != Novae::NOVA_OK)
		error(__func__, ":", __LINE__, " res=", res);

	/* destruct irq object */
	revoke(core_pd, Novae::Obj_crd(_kernel_caps, 1));
	cap_map().remove(_kernel_caps, 1);
}
...
static Range_allocator::Result allocate(Range_allocator &irq_alloc, Irq_args const &args)
{
	...
	if (args.type() == Irq_session::TYPE_LEGACY) {
		...
		return irq_alloc.alloc_aligned(1, Align { .log2 = 0 },
	...
	_irq_object(runtime, _irq_number)
{
	_irq_number.with_result(
		[&] (Range_allocator::Allocation const &a) {
			...
			_irq_object.start(unsigned(addr_t(a.ptr)), bdf, Irq_args(args));
		},
		[&] (Alloc_error) {
...
Genode::Thread::Start_result Irq_object::start()
{
	...
	bool const ec_created = _stack.convert<bool>([&] (Stack &stack) {
		...
		res = map_exception_portals(0, exc_pt_sel,
		...
		Pager_object::enable_delegation(exc_pt_sel, name != "pager");

		/* set infos during startup portal traversal, see platform.cc */
		Utcb & new_utcb = *reinterpret_cast<Utcb *>(&stack.utcb());
		new_utcb.ip(addr_t(global_irq_thread_entry));
		new_utcb.sp(stack.top());

		/* set information for startup portal for global IRQ threads in core */
		res = pt_ctrl(PT_SEL_STARTUP, addr_t(&new_utcb), Mtd::gpr_ip());
	...
	auto res = async_map(platform_specific().core_obj_sel(),
	                     platform_specific().core_obj_sel(),
	                     Obj_crd(PT_SEL_STARTUP, 0),
	                     Obj_crd(exc_pt_sel + PT_SEL_STARTUP, 0));
	                           ...
	                           private Core::Trace::Source
	{
		...
		Info trace_source_info() const override
		{
			...
			thread.with_native_thread([&] (Native_thread const &nt) {
				uint8_t res = Novae::sc_ctrl(nt.ec_sel + 2, sc_time);
				if (res != Novae::NOVA_OK)
