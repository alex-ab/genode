...
class Core::Ipc_pager
{
	...
	public:
		...
		Ipc_pager (Novae::Utcb &, addr_t pd_dst, addr_t pd_core, bool);
		 ...
		 */
		void reply_and_wait_for_fault(addr_t);
		 ...
		 */
		void set_reply_mapping(Mapping const &);
		...
		bool unknown_fault() const { return false; }
#else
		/**
		 * Return true if fault was a non-executable fault
		 */
		bool exec_fault() const {
			return ((_fault_type & ERR_PERMISSION) == ERR_PERMISSION) &&
			       !write_fault(); }

		/*
		 * ESR_EL1 register
		 */
		bool unknown_fault() const { return !(_fault_type & ERR_TRANSLATION); }
#endif
