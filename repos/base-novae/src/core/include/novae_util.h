...
/*
 * Copyright (C) 2010-2026 Genode Labs GmbH
 *
...
inline Novae::uint8_t map_exception_portals(Genode::addr_t const source_exc_base,
                                            Genode::addr_t const target_exc_base,
                                            Genode::addr_t const source_pd,
                                            Genode::addr_t const target_pd)
{
	...
	Novae::uint8_t res = Novae::NOVA_OK;

	for (auto pt_id = 0u; pt_id < (1u << Novae::NUM_PT_ARCH_LOG2); pt_id++) {
		Obj_crd const src(source_exc_base + pt_id, 0);
		Obj_crd const dst(target_exc_base + pt_id, 0);

		auto const res_tmp = async_map(source_pd, target_pd, src, dst);
