...
struct Page_fault_info
{
	...
	addr_t const pf_type;

	Page_fault_info(char const *pd, char const *thread, unsigned cpu,
	                addr_t ip, addr_t addr, addr_t sp, addr_t type)
	:
...
void Pager_object::_page_fault_handler(Pager_object &obj, unsigned mtd)
{
	...
	obj._state.thread.cpu.sp     = ipc_pager.sp();
#ifdef __x86_64__
	obj._state.thread.cpu.trapno = PT_SEL_PAGE_FAULT;
#else
	obj._state.thread.cpu.esr_el1 = ipc_pager.fault_type();
#endif
	...
	if (!ipc_pager.unknown_fault() &&
	    obj.pager(ipc_pager)       == Pager_object::Pager_result::CONTINUE &&
	    ipc_pager.syscall_result() == Novae::NOVA_OK) {

		obj._state.unblock_pause_sm();
		obj._state.unblock();
		obj._state_lock.release();
		ipc_pager.reply_and_wait_for_fault(myself.mystack().top);
	}
	obj._state_lock.release();

	Page_fault_info const fault_info(obj.client_pd(), obj.client_thread(),
	                                 platform_specific().pager_index(myself.affinity()),
	                                 ipc_pager.fault_ip(),
	...
	/* revoke page fault cap */
	revoke(obj.pd_sel_obj(), Obj_crd(obj._exc_pt_base_child + PT_SEL_PAGE_FAULT, 0));
	revoke(core_pd         , Obj_crd(obj._exc_pt_base_core  + PT_SEL_PAGE_FAULT, 0));

	/* kernel will de-schedule thread forever at next attempt to resolve the page fault */
	error(fault_info);
	error(" thread is dead, failed=", ipc_pager.syscall_result(),
	      " ", ipc_pager.unknown_fault() ? " unknown (alignment?) fault " : "");
...
void Pager_object::_recall_handler(Pager_object &obj, unsigned mtd)
{
	...
	reply(Thread::mystack().top, mtd, sm);
}
...
//				if (cnt == 0) chk_pd_1 = e.dst_pd;
	...
	case TAKE: /* take cap */
		...
		if (!(transfer.valid && transfer.id == id_trans)) {
			...
			msg_1 = Capability_space::INVALID_INDEX;
			mtd += 1;
			break;
		}
...
{
	Mtd const mtd (Mtd::gpr_ip() | Mtd::sp());
...
#else
	register_handler<0x00>(obj, mtd);
	register_handler<0x01>(obj, mtd);
	register_handler<0x02>(obj, mtd);
	register_handler<0x03>(obj, mtd);
	register_handler<0x04>(obj, mtd);
	register_handler<0x05>(obj, mtd);
	register_handler<0x06>(obj, mtd);
	register_handler<0x07>(obj, mtd);
	register_handler<0x08>(obj, mtd);
	register_handler<0x09>(obj, mtd);
	register_handler<0x0a>(obj, mtd);
	register_handler<0x0b>(obj, mtd);
	register_handler<0x0c>(obj, mtd);
	register_handler<0x0d>(obj, mtd);
	register_handler<0x0e>(obj, mtd);
	register_handler<0x0f>(obj, mtd);

	register_handler<0x10>(obj, mtd);
	register_handler<0x11>(obj, mtd);
	register_handler<0x12>(obj, mtd);
	register_handler<0x13>(obj, mtd);
	register_handler<0x14>(obj, mtd);
	register_handler<0x15>(obj, mtd);
	register_handler<0x16>(obj, mtd);
	register_handler<0x17>(obj, mtd);
	register_handler<0x18>(obj, mtd);
	register_handler<0x19>(obj, mtd);
	register_handler<0x1a>(obj, mtd);
	register_handler<0x1b>(obj, mtd);
	register_handler<0x1c>(obj, mtd);
	register_handler<0x1d>(obj, mtd);
	register_handler<0x1e>(obj, mtd);
	register_handler<0x1f>(obj, mtd);

	/* 0x20 - Instruction abort - lower level */
	register_handler<0x21>(obj, mtd);
	register_handler<0x22>(obj, mtd);
	register_handler<0x23>(obj, mtd);
	/* 0x24 - Data abort - lower level */
	register_handler<0x25>(obj, mtd);
	register_handler<0x26>(obj, mtd);
	register_handler<0x27>(obj, mtd);
	register_handler<0x28>(obj, mtd);
	register_handler<0x29>(obj, mtd);
	register_handler<0x2a>(obj, mtd);
	register_handler<0x2b>(obj, mtd);
	register_handler<0x2c>(obj, mtd);
	register_handler<0x2d>(obj, mtd);
	register_handler<0x2e>(obj, mtd);
	register_handler<0x2f>(obj, mtd);
	register_handler<0x30>(obj, mtd);
	register_handler<0x31>(obj, mtd);
	register_handler<0x32>(obj, mtd);
	register_handler<0x33>(obj, mtd);
	register_handler<0x34>(obj, mtd);
	register_handler<0x35>(obj, mtd);
	register_handler<0x36>(obj, mtd);
	register_handler<0x37>(obj, mtd);
	register_handler<0x38>(obj, mtd);
	register_handler<0x39>(obj, mtd);
	register_handler<0x3a>(obj, mtd);
	register_handler<0x3b>(obj, mtd);
	register_handler<0x3c>(obj, mtd);
	register_handler<0x3d>(obj, mtd);
	register_handler<0x3e>(obj, mtd);
	register_handler<0x3f>(obj, mtd);
#endif
}
...
void Pager_object::_construct_pager()
{
	...
	_exceptions.register_handler<14>(*this, Mtd::qual() | Mtd::gpr_ip() | Mtd::sp(),
	                                 _page_fault_handler);
#else
	_exceptions.register_handler<0x20>(*this, Mtd::qual() | Mtd::gpr_ip() | Mtd::sp(),
	                                   _page_fault_handler);
	_exceptions.register_handler<0x24>(*this, Mtd::qual() | Mtd::gpr_ip() | Mtd::sp(),
	                                   _page_fault_handler);
#endif

	/* create portal for recall handler */
	Mtd const mtd_recall(Mtd::gpr_ip() | Mtd::sp() | Mtd::flags_status());
	...
	_exceptions(*this)
{
	...
	_exceptions.register_handler<PT_SEL_STARTUP>(*this, Mtd::gpr_ip(),
	                                             _startup_handler);

	_exceptions.register_handler<PT_SEL_DELEGATE>(*this, Mtd::gpr_ip(),
	                                              _delegate_handler);
	/*
...
void Pager_object::enable_delegation(addr_t const pt_base, bool doit)
{
	...
	auto fn = [&](auto const base) {
		pager_threads[0]->with_native_thread([&](auto const &nt) {
		...
		if (!imprint_badge(platform_specific().core_obj_sel(),
			...
			error(__func__, ":", __LINE__, " imprint failed");
	};
