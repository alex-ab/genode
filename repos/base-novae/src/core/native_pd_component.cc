...
{
	if (!_pd_session._consume_cap(Pd_session_component::RPC_CAP))
		return Alloc_error::OUT_OF_CAPS;

	Alloc_rpc_cap_result result = _pd_session._rpc_cap_factory.alloc(reinterpret_cast<Runtime &>(_pd_session), ep, entry, mtd);
...
{
	if (cap.valid())
		imprint_badge(Core::platform_specific().core_obj_sel(), cap.local_name(), badge);
}
