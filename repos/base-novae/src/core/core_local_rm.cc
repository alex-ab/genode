...
Core_local_rm::attach(Dataspace_capability ds_cap, Attach_attr const &attr)
{
	return _ep.apply(ds_cap, [&] (Dataspace_component * const ds_ptr) -> Result {
		...
		/* map it */
		Novae::Rights const rights(true, attr.writeable && ds.writeable(),
		                           attr.executable);
		...
		auto res = map_local(Platform::kernel_host_sel(),
		                     ...
		                     page_rounded_size >> Genode::PAGE_SIZE_LOG2,
		                     rights);
...
void Core_local_rm::_free(Attachment &a)
{
	...
	unmap_local(platform_specific().core_host_sel(), addr_t(a.ptr),
	            size >> Genode::PAGE_SIZE_LOG2, Novae::Rights::none());
