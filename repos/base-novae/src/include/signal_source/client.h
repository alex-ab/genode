...
class Genode::Signal_source_client : public Rpc_client<Novae_signal_source>
{
	...
	public:
		...
		: Rpc_client<Novae_signal_source>(static_cap_cast<Novae_signal_source>(cap))
		{
			...
			myself->with_native_thread([&](auto const &nt) {
				request_signal_sm_cap(nt.exc_pt_sel + Novae::PT_SEL_PAGE_FAULT,
				                      nt.exc_pt_sel + Novae::SM_SEL_SIGNAL);

				_sem = Capability_space::import(nt.exc_pt_sel + Novae::SM_SEL_SIGNAL);
