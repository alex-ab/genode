/*
 * \brief  NOVAe-specific implementation of the Thread API
 * \author Norman Feske
...
/* NOVAe includes */
#include <novae_native_cpu/client.h>
...
void Thread::_init_platform_thread(size_t weight, Type type)
{
	...
	if (type == MAIN) {
		...
		native_thread().ec_sel     = Novae::EC_SEL_THREAD;
...
void Thread::_deinit_platform_thread()
{
	...
	if (native_thread().ec_sel != Native_thread::INVALID_INDEX) {
		error(__func__, " implement me - freeing missing");
...
Thread::Start_result Thread::start()
{
	...
	try {
		...
		Novae_native_cpu_client native_cpu(_cpu_session->native_cpu());
	...
	native_thread().ec_sel = native_thread().exc_pt_sel + Novae::EC_SEL_THREAD;
