...
struct Cpu::View
{
	...
	static char const * policy_to_short(enum POLICY const policy)
	{
		...
		case POLICY_MAX_UTILIZE: return "LEA";
	...
	short   hover_component    { 0 };
	short   selected_component { 0 };
	short   hover_thread       { 0 };
	short   selected_thread    { 0 };
	...
	void _gen_menu_entry(Xml_generator &, short,
...
void Cpu::View::handle_config()
{
	...
	_dialog.construct(_env, "dialog", "dialog", 4096 * 2);
	_dialog->enabled(true);

	_cpu_config.construct(_env, "config", "components", 4096 * 2);
...
void Cpu::View::handle_hover()
{
	...
	if (selected_thread && selected_component) {
		...
		_policy.hovered = query_attribute<unsigned>(node, "dialog", "frame",
...
void Cpu::View::_gen_menu_entry(Xml_generator &xml, short const id,
...
void Cpu::View::gen_config(Reporter::Xml_generator &xml, Xml_node const &input)
{
	...
	input.for_each_sub_node("component", [&](Xml_node const &node) {
		...
		node.for_each_sub_node("thread", [&](Xml_node const &node) {
			short const thread_id = thread_cnt++;
				...
				if (selected_thread && thread_id == selected_thread) {
					...
					gen_named_node(xml, "hbox", "select", [&] {
						xml.node("vbox", [&] () {
							...
							_gen_menu_entry(xml, POLICY_MAX_UTILIZE, "Least utilized CPU", policy == POLICY_MAX_UTILIZE);
						});
					});
