	...
	public:
		            ...
		            Genode::Constructible<Top::Storage> &storage)
		{
			...
			if (storage.constructed())
				storage->write(Type_a { ~0U /* invalid - means data start */,
			...
			for (unsigned i = 0; i < _num_subjects; i++) {
				...
				if (!thread) {
					...
					/* XXX - right place ?! */
					if (storage.constructed())
						storage->write(Type_b{thread->id(), thread->thread_name()});
				}
			...
			if (storage.constructed()) {
				for (unsigned i = 0; i < _num_subjects; i++) {
					...
					Top::Thread * thread = _lookup_thread(id);
					if (!thread) {
						Genode::warning("lost thread, should never happen");
						continue;
					}
...
void App::Main::_handle_hover()
{
	...
	if ((period_trace != _period_trace) || (period_view != _period_view)) {
		...
		if (period_view < 100)
			period_view = 100;
		if (period_trace == _period_trace) /* on view change adjust ever trace */
