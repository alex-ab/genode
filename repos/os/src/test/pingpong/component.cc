...
{
	Genode::Env            &_env;
	Genode::Rpc_entrypoint  _rpc_server;
	Test::Component         _comp       { };
	Test::Capability        _rpc_cap    { _rpc_server.manage(&_comp) };
	Test::Client            _client     { _rpc_cap };

	Intra_pd(Genode::Env &env, Genode::Affinity::Location location,
	         bool use_tracing)
	:
		Tracing(env),
		_env(env),
		_rpc_server( &_env.pd(), 4 * 4096, "call_me", location)
	{
...
void Component::construct(Genode::Env &env)
{
	...
	bool const test_xcore  = config.xml().attribute_value("xcore", false);

	Genode::log("config: ", use_tracing ? " - use tracing " : "",
	                        test_xcore ? " - cross core IPC " : "");

	Genode::Affinity::Space space = env.cpu().affinity_space();
	Genode::Affinity::Location location = space.location_of_index(test_xcore ? 1 : 0);

	Intra_pd intra_pd(env, location, use_tracing);
