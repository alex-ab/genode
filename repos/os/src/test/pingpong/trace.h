...
*/
	                      ...
	                      Genode::Trace::Timestamp const time_start)
	{
		...
		Genode::log("\nU->C->D->Y->R->U");
		Genode::log(" U - user call    starts   (client)");
		Genode::log(" C - RPC call     starts   (client)");
		Genode::log(" D - RPC dispatch invoked  (server)");
		Genode::log(" Y - RPC reply    invoked  (server)");
		Genode::log(" R - RPC call     returned (client)");
		Genode::log(" U - user call    returned (client)");
		Genode::log("deviation:");
		Genode::log(" error shown, a-f, when deviation is > 10\% of middle");
		Genode::log(" - (a) C->D->Y->R");
		Genode::log(" - (b) C->D");
		Genode::log(" - (c) D->F->Y");
		Genode::log(" - (d) Y->R");
		Genode::log(" - (e) U->C");
