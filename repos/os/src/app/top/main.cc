	...
	public:
		            ...
		            Genode::Allocator &alloc)
		{
			...
			for_each_thread([&] (Top::Thread &thread) {
				...
				total[x][y] += thread.recent_execution_time();
				...
				for (unsigned i = 0; i < _config_max_elements_per_cpu; i++) {
					if (thread.recent_execution_time()
		...
		Genode::String<128> string(Top::Thread const &thread,
		                           unsigned long long const total)
		{
			return string(thread, total, true, true);
		}
		...
		{
			unsigned const percent = thread.recent_execution_time() * 100   / total;
			unsigned const rest    = thread.recent_execution_time() * 10000 / total - (percent * 100);

			Genode::String<8> text_str(text ? string(percent, rest) : "");

			return Genode::String<128>("cpu=", thread.affinity().xpos(), ".",
		...
		void top()
		{
			for_each([&] (Top::Thread const &thread, unsigned long long const total) {
				Genode::log(string(thread, total));
			});
		           ...
		           Genode::Trace::Subject_id const id)
		{
			if (click_valid) {
				...
				if (_button_reset_graph_hovered) {
					for_each_thread([&] (Top::Thread &thread) {
						if (thread.track()) thread.track(false);
					});
					_tracked_threads = 0;
					update = true;
				}
				...
				xml.node("hbox", [&] () {
					...
					xml.node("vbox", [&] () {
						...
						xml.node("hbox", [&] () {
							...
							if (_sort == THREAD) list_view(xml);
							if (_sort == COMPONENT) list_view_pd(xml);
						});
					});
				});
			});
		...
		void graph(Genode::Reporter::Xml_generator &xml)
		{
			for_each_thread([&] (Top::Thread &thread) {
				if (!thread.track())
					return;
				...
				xml.node("entry", [&]{
					...
					Genode::String<12> cpu_name(xpos, ".", ypos);
					xml.attribute("cpu", cpu_name);
					xml.attribute("label", thread.session_label());
					xml.attribute("thread", thread.thread_name());

					Genode::uint64_t t = total[thread.affinity().xpos()][thread.affinity().ypos()];

					xml.attribute("value", t ? (thread.recent_execution_time() * 10000 / t) : 0 );
				});
			});
		}
				...
				xml.node("hbox", [&] () {
					...
					detail_view_tool(xml, thread, Genode::String<16>("load "), 3,
						[&] (Top::Thread const &e, bool &left) {
							unsigned long long t = total[e.affinity().xpos()][e.affinity().ypos()];
					...
					xml.node("vbox", [&] () {
						...
						thread.for_each_thread_of_pd([&] (Top::Thread &check) {
							xml.node("button", [&] () {
								xml.attribute("name", check.id().id * DIV + 9);
		...
		void list_view(Genode::Xml_generator &xml)
		{
			xml.node("vbox", [&] () {
				...
				for_each([&] (Top::Thread const &thread, unsigned long long const total) {
					...
					unsigned percent = thread.recent_execution_time() * 100 / total;
					unsigned rest    = thread.recent_execution_time() * 10000 / total - (percent * 100);
		...
		void list_view_pd(Genode::Xml_generator &xml)
		{
			...
			xml.node("vbox", [&] () {
				...
				xml.node("hbox", [&] () {
					xml.attribute("name", "load");
					xml.node("label", [&] () {
						xml.attribute("text", Genode::String<16>("load cpu", _last_cpu.xpos(), ".", _last_cpu.ypos()));
						xml.attribute("color", "#ffffff");
					});
				});
				...
				for_each_pd([&] (Genode::Avl_string_base const &base) {
					...
					thread.for_each_thread_of_pd([&] (Top::Thread &t) {
						if (_same(t.affinity(), _last_cpu))
							time += t.recent_execution_time();
					});

					Genode::uint64_t max = total[_last_cpu.xpos()][_last_cpu.ypos()];
				...
				_pd_scroll.last = pd_count; /* adjust count on every run */
			});
...
void App::Main::_handle_hover()
{
	...
	if (_trace_subject_registry.period() != _period_ms) {
		_period_ms = _trace_subject_registry.period();
		if (_period_ms < 10) {
			_period_ms = 1;
			_trace_subject_registry.period(_period_ms);
		}
	...
	if (!id.id) {
		id = query_attribute<unsigned>(hover, "dialog", "frame", "vbox",
		                               "hbox", "vbox", "button", "name") / DIV;
	}

	if (_trace_subject_registry.hover(button, click, click_valid, id) && click_valid)
		_generate_report();
}
...
void App::Main::_generate_report()
{
		...
		do {
			try {
				Reporter::Xml_generator xml(*_reporter_graph, [&] () {
					_trace_subject_registry.graph(xml);
				});
	...
	_empty_graph = !_trace_subject_registry.tracked_threads();
}
