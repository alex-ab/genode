...
class Graph
{
	private:
		...
		Genode::Trace::Subject_id     _id[MAX_GRAPHS];
		bool                          _id_valid[MAX_GRAPHS];
		Genode::String<12>            _cpu[MAX_GRAPHS];
	...
	public:
		...
		Graph(Genode::Env &env) : _env(env)
		{
			...
			Genode::memset(_id, 0, sizeof(_id));
			Genode::memset(_id_valid, 0, sizeof(_id_valid));
...
void Graph::_apply_test_data(int * values, Genode::Color const &color,
                             ...
                             unsigned entry)
{
	unsigned const step = (_width - _x_root) / (_column_cur + 1);
...
void Graph::_replay_data()
{
	for (unsigned i = 0; i < _column_cur; i++) {
		for (unsigned graph = 0; graph < MAX_GRAPHS; graph++) {
			Nitpicker::Point point = _column[i]._points[graph];
			if (!point.x() && !point.y())
				continue;

			Genode::Color color = _color(graph);
			int value = _column[i]._values[graph];
			point = _apply_data_point(value, i);

			marker(point, _marker_half, color);

			_column[i]._points[graph] = point;
			_column[i]._values[graph] = value;
		}
	}
}
...
{
	Nitpicker::Point point = _apply_data_point(value, _column_cur);
	marker(point, _marker_half, color);

	_column[_column_cur]._points[entry] = point;
	_column[_column_cur]._values[entry] = value;
	_column[_column_cur]._used ++;
}

bool Graph::advance_column_by_storage(Genode::uint64_t const time)
{
	if (time > this->time()) {
//		Genode::warning("advance ", Genode::Hex(time), " ", Genode::Hex(_time_storage_wait_for));
...
bool Graph::new_data(unsigned const value, unsigned const id, Genode::uint64_t const tsc)
{
	...
	for (unsigned graph = 0; graph < MAX_GRAPHS; graph++) {
		if (!_id_valid[graph] || !(_id[graph] == Genode::Trace::Subject_id(id)))
			continue;
...
//	Genode::warning("_handle_data ", _column_last, "->", _column_cur);
	...
	_nitpicker.framebuffer()->refresh(xpos_s - _marker_half, 0,
...
void Graph::_handle_input()
{
	...
	if (hovered && _hovered_vline != hovered_old) {
		...
		for (unsigned i = 0; i < MAX_GRAPHS; i++) {
			...
			unsigned const cmp = _cpu[i].length() > 6 ? 8 : 5;
...
void Graph::apply_test_data()
{
	...
	_column_cur = sizeof(values_a) / sizeof(values_a[0]);
	if (_column_cur >= sizeof(_column) / sizeof(_column[0])) {
		Genode::error("too many elements in graph ", _column_cur);
		return;
	}

	Genode::String<12>         cpu_a("0.0");
	Genode::Trace::Thread_name thread_a("ep");
	Genode::Session_label      session_a("init -> menu_view");
	Genode::String<12>         cpu_b("1.0");
	Genode::Trace::Thread_name thread_b("timer");
	Genode::Session_label      session_b("init -> timer");
	Genode::String<12>         cpu_c("2.0");
	Genode::Trace::Thread_name thread_c("ep");
	Genode::Session_label      session_c("init -> nitpicker");

	_apply_test_data(values_a, _color(0), thread_a, session_a, cpu_a, 35, 0);
	_apply_test_data(values_b, _color(1), thread_b, session_b, cpu_b, 40, 1);
	_apply_test_data(values_c, _color(2), thread_c, session_c, cpu_c, 43, 2);
