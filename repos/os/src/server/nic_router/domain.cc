...
{
	bool result = true;
	node.for_each_sub_node(type, [&] (Node const &node) {
		if (!result)
			return;
...
bool Domain::init(Domain_dict &domains)
{
	...
	bool result = true;
	_node.with_optional_sub_node("dhcp-server", [&] (Node const &dhcp_server_node) {
	...
	/* read NAT rules */
	_node.for_each_sub_node("nat", [&] (Node const &node) {
		if (!result)
			return;
	...
	/* read ICMP rules */
	_node.for_each_sub_node("icmp", [&] (Node const &node) {
		if (!result)
			return;
	...
	/* read IP rules */
	_node.for_each_sub_node("ip", [&] (Node const &node) {
		if (!result)
			return;
