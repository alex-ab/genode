...
class Vdi::File: public Vfs::Io_response_handler
{
	private:
		...
		enum Read { NONE, READ, LOOP_READ, CHECK };
				...
				case Vfs::File_io_service::READ_OK:
				{
					if (_state_fs_read.remaining != n) {
						if (n)
							...
							/* should not happen, avoid endless loop by _read() caller */
							Genode::warning("READ_OK with size zero");
					}
	...
	public:
		...
		bool init(Genode::Signal_context_capability const init_signal)
		{
			...
			Genode::log("block_size: ", _block_ops.block_size, " block_count: ",
			            _block_ops.block_count);
