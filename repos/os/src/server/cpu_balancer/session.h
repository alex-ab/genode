	...
	private:
		...
		void construct(Cpu::Policy::Name const &policy_name, FUNC const &fn)
		{
			...
			try {
				...
				if (thread) {
					Genode::error("memory leak avoided - remove this debug message");
		...
		/* XXX better way to do it in constructor context ? */
...
//			Genode::error("extra ram ", extra_ram, " '", args, "' -> ", (char const *)argbuf);
	...
	public:
		...
		void upgrade(Root::Upgrade_args const &args, FUNC const &fn) {
			/* XXX how to distinguish between parent requests and own ? */
